...
router.post('/upload', upload.single('mp3file'), async (req, res) => {
    if (!req.file) {
        return res.status(400).json({ detail: "Geen bestand geüpload." });
    }
    const tempPath = req.file.path;
    try {
        const tags = NodeID3.read(tempPath);
        const newSong = await Song.create({
            title: tags.title || req.file.originalname.replace('.mp3', ''),
        ...
        res.status(500).json({ detail: "Fout bij verwerken MP3." });
    }
});
...
// ==========================================================
// NIEUW - Verwijder een nummer (permanent)
// ==========================================================
router.delete('/songs/:songId', async (req, res) => {
    ...
    try {
        ...
        // 2. Verwijder uit database (Sequelize regelt de playlist-associaties)
        await song.destroy();
